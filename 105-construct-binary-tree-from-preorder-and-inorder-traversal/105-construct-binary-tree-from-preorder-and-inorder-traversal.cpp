...
class Solution
{
    public:
        TreeNode* solve(vector<int> &preorder, int preStart, int preEnd, vector<int> &inorder, int inStart, int inEnd, unordered_map< int, int> &mp)
        {
            ...
            root->left = solve(preorder,preStart+1,preStart+leftNums,inorder,inStart,inRoot-1,mp);
            root->right = solve(preorder,preStart+leftNums+1,preEnd,inorder,inRoot+1,inEnd,mp);
